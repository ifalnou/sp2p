...
use crate::core::crypto::{noise_server_handshake, read_noise, write_noise};
use crate::core::notifications::FileReceivedEvent;
use crate::core::state::{ActiveTransfer, TransferHistoryItem, GLOBAL_STATE};

pub fn spawn_server(
    port: u16,
    inbox_root: PathBuf,
    crypto_key: Arc<[u8; 32]>,
    notify_tx: mpsc::UnboundedSender<FileReceivedEvent>,
) {
        ...
        loop {
            match listener.accept().await {
                Ok((socket, addr)) => {
                    ...
                    tokio::spawn(handle_connection(socket, addr, root, psk, notif_tx));
                }
...
async fn handle_connection(
    ...
    let dest_dir = inbox_root.join(&inbox_name);
    if !dest_dir.exists() {
